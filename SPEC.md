...
There is **no consent banner and none is needed**: Umami stores nothing on the device, so TDDDG §25 is not triggered and the processing rests on Art. 6(1)(f). That is the whole basis for the current `/cookies` and Datenschutz §6 copy. **Do not swap in a tool that writes cookies or localStorage** (Google Analytics, Matomo with cookies, Microsoft Clarity) without adding a consent gate *before* load and rewriting `cookiesPage.*` + `privacyPage.*` in all five locales. Verified 2026-08-01 in a headless browser: Umami sets no cookie, no `localStorage`, no `sessionStorage`, and honours `navigator.doNotTrack` (collection POST fires with DNT off, not at all with DNT on). **The app sets no cookies of its own** — `routing.ts` sets `localeCookie: false`, because with `localeDetection: false` and no middleware nothing ever read `NEXT_LOCALE`. The *platform* does: Azure SWA returns `ARRAffinity` + `ARRAffinitySameSite` session cookies on every response (verified 2026-08-02 with `curl -I` against the deployed host). They are load-balancing cookies, exempt under TDDDG §25(2) no. 2, and are now named as such in `privacyPage.cookiesLead` and `cookiesPage.essentialBody` instead of being hedged as cookies that "may" be set.

Umami Cloud is operated by **Umami Software, Inc. (San Francisco, USA)**, and the GRAEWE account is configured with the **EU data region**, so collected data is stored on EU servers. Datenschutz §6 states both, plus the SCCs from Umami's DPA as the safeguard for any US access. **The data region is account configuration, not repo state** — nothing here can detect it being changed, so if anyone switches the account to US, §6 becomes wrong in all five locales.

**The privacy policy names only processors that actually run.** Turnstile and Resend are implemented but unconfigured in production (no `TURNSTILE_SITE_KEY` var, no `TURNSTILE_SECRET_KEY` / `RESEND_API_KEY` secret), so they are deliberately absent from `privacyPage.*`: a processor that processes nothing cannot be under-disclosed, whereas naming an inert one states something untrue in a binding document. `.env.example` carries the warning — setting either key must add the processor to section 3 in all five locales in the same change. The two are not symmetrical, though: **Resend is pending** (blocked on DNS handover, then the contact form needs it — ISSUE-065), while **Turnstile is decided against** (2026-08-03, ISSUE-066): the spam protection that ships is the honeypot plus a best-effort per-IP rate limit, and the Turnstile code stays in the tree unreachable. Do not "finish the setup" by filling in its keys. The same rule produced Datenschutz §7: the Vimeo player in `HomeVideo.tsx` is click-to-load with a local thumbnail and `dnt=1`, so nothing reaches Vimeo until the user clicks, and that click is the Art. 6(1)(a) / TDDDG §25(1) consent. **Do not make that embed auto-load** — it would move a US transfer to page load with no consent behind it.

Note that `cloud.umami.is` (script) and `gateway.umami.is` (ingest) are shared global hosts and say nothing about the storage region — the host name is not evidence either way. Confirm the region in the Umami dashboard, not by probing endpoints.
