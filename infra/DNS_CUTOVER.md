...
- `src/proxy.ts` — applies 301s at request time (incl. `.html` and query URLs)
- `staticwebapp.config.json` — path-only 301s for Azure SWA (regenerated by unit tests from the same map)
